...
- 输水道附属区（convoy2b）「收割者」登场过场字幕修复：4 条坦克无线电（vo_1_2_12_30_1~4，斑马 2/野牛 8）被过场镜头距离门控误杀（镜头距 speaker 1100+ 超过 maxDist 600，speaker 的 s_global 只写实体 parms 而 shader decl 无 global 标志），过场白名单扩展为前缀数组并加入 vo_1_2_12_30_；同组补说话人映射（_2「斑马 2」为应答方推断，_3/_4「野牛 8」由紧随其后的 #str_303755"野牛 8 报告遇袭"对话链推断，与 _1 一致）
- 施工区（walker）PA 字幕误显示"沃克："修复：speaker 实体名 walkerPA_1~5 含地图名"walker"，被 AddFromEntity 实体名子串匹配（knownSpeakers）误判为角色 Walker → 人名映射成"沃克"+白色。修复：speaker 实体的声音名 speaker_map 映射查询提前到实体名匹配之前（声音语义优先于实体名，无映射声音不受影响）
- 医疗设施（medlabs）设施播报误标"舰载广播"修复：vo_2_1_1_05_1"行为程序已下载至神经控制体"为 Strogg 设施播报但命名非 vo_pa_ 前缀，fallback 按前缀误判为人类舰船广播（蓝）。broadcast_pa.tsv 补该条目（str_386113，敌军广播/黄色），经上一条的声音映射优先修复正常显示
- 清理 dist/english_lipsadd.lang 中两条无 TSV 来源、无 decl 引用的陈旧条目（随 build_lang 重建管线归位）
- speaker_map 183 条角色映射；q4game.dll 同步部署 idTech4Apx/dist/游玩目录，官方备份哈希未动

## v1.3.5 — 2026-08-07

### 过场字幕门控 + 怪物 alert 禁用 + Voss boss 补全

- 过场（cinematic）字幕门控：过场期间过滤外部 NPC 与背景广播的字幕乱入，只保留过场本身的内容——过场角色对白（cinematic 标志实体）、GLOBAL 全局广播（如开场飞船广播）、mcc_2 飞船被攻击广播（白名单 vo_2_2_10_251_*）照常显示；远处背景声音（如 mcc_1 会议过场的走廊对白 talkLooms）按离镜头距离（声音自身 maxDistance）过滤
- 怪物 alert 字幕禁用：注释 6 个 alert lipsync decl（狂战士/机炮兵/铁娘子/科学家/失败改造体），怪物发现喊话（"我看到你了""啊哈，准备受死吧"等）不再显示字幕
- Voss 改造后 boss 对白补全：补 4 句中文翻译（"凯恩，你必须离开这里！""快走！离开这里！""我控制不住了……快跑！！！"）+ 沃斯说话人推断，废物处理设施 boss 战 5 句残存意识对白全部有字幕
- alert 过场过滤：过场期间非过场怪物的 alert 喊话不再乱入
- lipsync decl 去重：清理 76 个 speaker/radio 重复定义 + 2 个误加 decl，避免引擎加载到错误版本
- 字幕颜色注释修正（舰载广播蓝 / 敌方广播黄 / 无线电绿）

## v1.3.4 — 2026-08-06

### 碎尸增强 + 字幕大幅改进

- 碎尸：标准 pork 模型（big_ 缺失修复）+ 人型配置（头/躯干/双臂/双腿/骨盆/骷髅/脑浆）+ 体型分级（小10/中12/大15）+ 脑浆模型 + cvar 默认关
- alert 怪物名：显示具体怪物名（机炮兵/铁娘子等红色）+ 台词统一"我看到你了" + 补狂战士 + 铁娘子"必须……杀……"
- 武器改造粉红色：12 个武器改造/获取声音标粉红色（说话人前缀 + 粉红色）
...
### 击杀碎尸系统 + 敌军广播 + 受伤音去除 + 安装器修复

- 击杀碎尸：武器限制（霰弹/榴弹/钉枪/轨道/火箭/闪电）+ 体型分级（小怪 10 块、中型 12 块、大型 15 块大肉块）+ 尸块沿打击方向抛物线飞溅 + boss/机械/失败改造体排除；安装器新增击杀碎尸选项（默认关闭）
- 字幕改进：Strogg 广播前缀改为「敌军广播」，飞船广播显示蓝色；去掉队友受伤音/死亡音字幕；数据存储终端补 3 条敌方广播字幕
- 人名汉化修复：恢复丢失的人名汉化系统（247 个人名映射）；修复 Sledge 名字误带「」
- 安装器修复：修复人名汉化选项安装时未禁用的 bug；修正部署位置（savedata/q4base）

## v1.3.2 — 2026-08-06

### 人名汉化 + 字幕去重 + 马克龙修复

人名汉化系统（harm_g_cnNames）:
- 新增人名汉化开关（默认开启），字幕前缀、字幕内容、准心队友名全部中文显示
- 覆盖 200+ 角色名/地名/小队名的运行时翻译映射
- 军衔位置统一为"姓名+军衔"（如"莫里斯中士"），消除"中士莫里斯"的不一致
- 准心两行显示中文名+军衔和小队名（小队名加「」与姓名区分）
- 敌方单位名和小队名在字幕内容中用「」包裹（如「机炮兵」「犀牛」小队）
- TSV 翻译批量修复 1255 处英文名残留

字幕修复:
- 修复敌方字幕重复显示（去重逻辑改为比较纯内容，不比较含前缀的完整文本）
- 去掉马克龙笑声字幕
- 去掉队友受伤音/死亡音的字幕
- 修复 PA 广播 decl 错误引用马克龙 str_id（v1.3.1 str_id 冲突修复的遗留）

引擎改动:
- 安装器窗口高度增大（v1.3.1 后续修复）
- 字幕 demo 模式（harm_g_subtitleDemo）用于测试所有字幕样式

## v1.3.1 — 2026-08-04

### 字幕修复 + 帧率解锁 + Strogg 文字修复

帧率解锁:
- 安装器新增"解锁帧率"选项（默认勾选），游戏可超过 60 FPS（需显示器支持高刷新率）
- 原理：com_fixedTic -1（idTech4A++ 原生支持），通过 autoexec.cfg 预设

字幕修复:
- 修复同一句话显示两条字幕（说话人/颜色不一致）
- 修复 Makron Boss 嘲讽字幕不显示（动画帧事件绕过 AI::Speak）
- 修复 str_id 冲突导致串台词（ai_vo_gap 与 radio_chatter/broadcast_pa 共 6 处冲突）
- 修复 Strogg 广播颜色错乱（vo_pa_ 前缀判断不准，改为默认黄色）
- 去掉网络守卫咆哮、数据流守卫嘶吼字幕（非台词音效）
- 无线电场景描述文字加括号区分

Strogg 文字修复:
- 修复改造后 Strogg 文字乱码/上下颠倒（用户目录 fontdat 旧残留与 TGA 不匹配）

## v1.3.0 — 2026-08-04

### 字幕系统全面修复 + 准心命中变红 + 喷血修复

字幕系统:
- 修复 #str_ 引用被跳过（v1.2.1 引入的 bug）
- lipsync decl 全面补齐至 1172 个，4 个 TSV 全部 100% 覆盖
- speaker_map.txt 首次纳入 installer payload
- 车队代号加「」包裹（20 处）
- 字幕颜色系统修复：subtitles.gui forecolor 绑定 subTxtR/G/B 变量
  - 无线电 → 绿色，人类/舰船广播 → 蓝色，Strogg 广播 → 黄色
  - 敌军语音 → 红色，Makron → 紫色，角色对白 → 白色
- Add 方法新增 colorOverride 参数：无线电字幕强制绿色（不受 speaker 名影响）
- Strogg AI 发现敌人时的 alert 字幕（8 种特殊单位：机炮兵/铁娘子/科学家等）
- Makron Boss taunt 翻译 9 条（"你会失败的！" "去死吧，人类！" 等）
- speaker_chatter.tsv 修复双 BOM

准心命中变红:
- 纯 q4game.dll 层面实现（weaponChange + crossColor），不编译引擎，不改 cursor.gui

喷血修复:
- 敌人角色（idActor）/bleed=true 实体 → 喷血
- 车辆（rvVehicle）/飞船（idAnimatedEntity 有 metal 材质）→ 喷火花
- 非 vehicle 的 idAnimatedEntity（补给器/尸体/墙上人彘等）→ 喷血
- GetDefaultSurfaceType 恢复 METAL，喷血由 HitScan/Projectile 的类型判断控制

引擎:
- 保留预编译 Quake4.exe + 旧版 SDL2.dll
- Sentry 物理修复：TestHugeTranslation 静默警告不清零速度（避免 Sentry 坠落）

字体:
- 同步 dist 字体文件到工程目录最新版（marine 原版基础段 + lowpixel/strogg/chain 全套）

已知问题:
- hub2（枢纽）关卡冷启动（map/devmap 跳关）可能卡顿，正常流程从头游玩不受影响
